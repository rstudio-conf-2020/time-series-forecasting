...
aus_livestock %>% filter(between(year(Month), 1992, 1996),
...
fc <- bricks %>% model(MEAN(Bricks)) %>% forecast(h = "5 years")
...
```{r dj9extra, echo=FALSE, fig.height=2.}
augment(fit) %>% ACF(.resid, lag_max=10) %>%
  autoplot() + ggtitle("ACF of residuals")
```

```{r dj9, echo=TRUE}
# lag=h and dof=K
ljung_box(augment(fit)$.resid, lag = 10, dof = 0)
```
 ...
 * Split the household wealth (`hh_budget`) into a training and test set, with the last four years in the test set.
