...
Repeat the daily electricity example, but instead of a using a linear function of temperature, use a piecewise linear function with the "knot" around 20 degrees Celsius. How can you optimize the choice of knot?
...
(calls <- readr::read_tsv("http://robjhyndman.com/data/callcenter.txt") %>%
  ...
  transmute(time = X1, date = as.Date(date, format = "%d/%m/%Y"),
...
insurance %>% pivot_longer(c(Quotes, TV.advert)) %>%
