...
\end{align*}\end{alertblock}\pause
  ...
  * If $\eta_t$ is non-stationary, the model is equivalent to regressing differenced $y_t$ on differenced $x_{1,t},\dots,x_{k,t}$.
...
\vspace*{-.2cm}
\begin{align*}
y_t-y_{t-1} &= `r format(drift, digits=2)` + 0.30\varepsilon_{t-1} + \varepsilon_t \\
...
rbind(fc_deterministic, fc_stochastic) %>%
  ...
  labs(
    x = "Year", y = "Visitors to Australia (millions)",
...
calls <- readr::read_tsv("http://robjhyndman.com/data/callcenter.txt") %>%
  pivot_longer(-X1, names_to = "date", values_to = "volume") %>%
  transmute(
...
```{r callsres, echo=TRUE, dev = "png", dpi = 180}
augment(fit) %>%
